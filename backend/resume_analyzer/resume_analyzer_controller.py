...
def analyze_resume(resume_uid: ResumeAnalyzerPayload):
    ...
    response = model.generate_content(f'''You are an expert resume reviewer. Your job is to provide feedback to the provided resume in the format: "
                                          ...
                                          "The resume feedback points need to be multiple sentence bullet points. I will give a resume and a job description, and then you will have to analyze it and provide a fit score for the job on the from 0 to 100. After finishing the bullet points, on the new line write score out of 100. The fit score should be in the format:<score>"
                                          "The feedback should also be provided with the words you, to specify that you are talking to a person. Focus on matching skills, experience, and education to the job description. The sentences should end with a period, but have no periods inside of sentences (for example the period inside of "e.g." is not valid).If "Node.js" or "Node js" is in the feedback, don't go to a new line after the "." followed by "js
